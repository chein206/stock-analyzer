...
def search_stocks(krx: pd.DataFrame, query: str):
    q = query.strip()
    if not q:
        return pd.DataFrame(columns=['Code', 'Name'])
    mask = (
        krx['Name'].str.contains(q, na=False, case=False) |
        krx['Code'].str.contains(q, na=False)
    )
...
def init_watchlist():
    ...
    if 'wl_save_cnt' not in st.session_state:
        st.session_state.wl_save_cnt = 0
...
def calc_zones(df: pd.DataFrame, info: dict) -> dict:
    ...
    return {
        ...
        'w52_low': w52_low, 'w52_high': w52_high,
...
def render_analysis(code: str, name: str, months: int):
    ...
    with c1:
        ...
        st.markdown(
            ...
            f"<div class='zone-sub'>추천 매수가 {int(z['buy_mid']):,}원 · 현재가 대비 {dp:+.1f}%</div>"
            f"</div>", unsafe_allow_html=True,
        )
    with c2:
        ...
        st.markdown(
            ...
            f"<div class='zone-sub'>매수가 대비 {sp:.1f}% · 이 이하면 손절</div>"
            f"</div>", unsafe_allow_html=True,
        )
    st.write("")
    c3, c4 = st.columns(2)
    with c3:
        ...
        st.markdown(
            ...
            f"<div class='zone-sub'>매수가 대비 +{t1p:.1f}% · 볼린저 상단</div>"
            f"</div>", unsafe_allow_html=True,
        )
    with c4:
        ...
        st.markdown(
            ...
            f"<div class='zone-sub'>매수가 대비 +{t2p:.1f}% · 52주 고점 부근</div>"
            f"</div>", unsafe_allow_html=True,
        )
